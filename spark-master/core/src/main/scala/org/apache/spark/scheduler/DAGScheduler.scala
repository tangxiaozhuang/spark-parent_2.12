...
private[spark] class DAGScheduler(
  ...
  def submitJob[T, U](
    ...
    //eventProcessLoop是一个监听对象，post方法会将JobSubmitted对像发送给dagSheduler一个后台监听线程
  ...
  private[scheduler] def cleanUpAfterSchedulerStop() {
    for (job <- activeJobs) {
      ...
      runningStages.toArray.foreach { stage =>
        eAsFinished(stage, Some(stageFailedMessage))
      }
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    //第一步：handleJobSubmitted方法一开始会定义一个空stage，由于是反向切割，所以这个stage实际上是程序执行的最后一个stage
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          //如果是头部的stage，即没有父stage，则开始进入划分任务并提交任务的操作
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
          submitMissingTasks(stage, jobId.get)
        } else {
          for (parent <- missing) {
            //这是一个递归操作，如果有父类rdd，则优先提交父类stage
            submitStage(parent)
          }
          //当前RDD被放在一个缓存
          waitingStages += stage
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    if (tasks.nonEmpty) {
      ...
      //将所有task放在一个taskSet集合，利用taskScheduler对象的submitTasks提交，这里submitTasks（）方法只是一个抽象类方法，具体的实现其实是TaskScheduler中的submitTasks方法
      taskScheduler.submitTasks(new TaskSet(
        tasks.toArray, stage.id, stage.latestInfo.attemptNumber, jobId, properties))
    } else {
      // Because we posted SparkListenerStageSubmitted earlier, we should mark
      // the stage as completed here in case there are no tasks to run
      eAsFinished(stage, None)
    ...
    event.reason match {
      ...
      case FetchFailed(bmAddress, shuffleId, mapId, _, failureMessage) =>
        ...
        if (failedStage.latestInfo.attemptNumber != task.stageAttemptId) {
          ...
          if (runningStages.contains(failedStage)) {
            ...
            eAsFinished(failedStage, errorMessage = Some(failureMessage),
      ...
      case failure: TaskFailedReason if task.isBarrier =>
        ...
        if (failedStage.latestInfo.attemptNumber != task.stageAttemptId) {
          ...
          eAsFinished(failedStage, Some(message))
        ...
        if (!stageIdToStage.contains(stageId)) {
          ...
          if (runningStages.contains(stage)) {
            try { // cancelTasks will fail if a SchedulerBackend does not implement killTask
              ...
              eAsFinished(stage, Some(failureReason))
            } catch {
              case e: UnsupportedOperationException =>
                logWarning(s"Could not cancel tasks for stage $stageId", e)
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  override def onReceive(event: DAGSchedulerEvent): Unit = {
    ...
    try {
      //监听线程收到提交job的事件，会调用doOnReceive方法
      doOnReceive(event)
    } finally {
      timerContext.stop()
    }
  ...
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
      //判断是否是job提交任务，执行handleJobSubmitted方法，切分stage
