...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint extends ThreadSafeRpcEndpoint with Logging {
    ...
    override def receive: PartialFunction[Any, Unit] = {
        ...
        if (TaskState.isFinished(state)) {
          executorDataMap.get(executorId) match {
            case Some(executorInfo) =>
              ...
              //如果之前的任务执行完成，则调用makeOffers准备提交新任务
      ...
      //接收到ReviveOffers类型对象，调用makeOffers()方法
      case ReviveOffers =>
        makeOffers()
    ...
    private def makeOffers() {
      ...
      val taskDescs = withLock {
        ...
        //将所有可用的executor封装成workOffers，每个workOffer代表代表每个executor所可用的资源数量
        ...
        //执行分配算法，将任务分配到相应的executor上去，得到分配结果集合
        scheduler.resourceOffers(workOffers)
      }
      if (taskDescs.nonEmpty) {
        //调用launchTasks方法，对分配后的结果进行下一步操作
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit() >= maxRpcMessageSize) {
          ...
          //根据executorId，从executorDataMap取出相应的executor。executorDataMap存储executor信息的集合，executor反向注册时的信息被存在这个集合中
  ...
  override def reviveOffers() {
    //driverEndpoint发送ReviveOffers，由recieve方法接受
