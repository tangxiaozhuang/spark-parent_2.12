...
private[spark] class TaskSchedulerImpl(
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
      //为每个taskSet创建一个TaskSetManager，用于追踪task的状态与及完成一些失败重试等工作
    ...
    //backend是taskScheduler初始化时创建的一个CoarseGrainedSchedulerBackend对象，
   ...
   * that tasks are balanced across the cluster.
    * 由群集管理器调用，提供从服务器上的资源。我们通过询问我们的主动任务来回应
    *
    * 按优先次序设置任务。我们以循环的方式向每个节点填充任务
    *
    * 任务在集群中是平衡的。
   */
